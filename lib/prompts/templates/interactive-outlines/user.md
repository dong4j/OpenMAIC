...
The built-in textbook context is the authoritative scope. If the user requirement is short, expand it only with matched textbook concepts, methods, cases, or activities. Do not introduce concepts, examples, or widget topics that are not supported by the textbook context.
